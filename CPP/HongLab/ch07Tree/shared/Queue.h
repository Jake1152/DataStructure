...
public:
	...
	{
		// 원형 큐에서 꽉 찼다의 기준
		return (rear_ + 1) % capacity_ == this->front_;
	}
	...
	int Size() const
	{
		...
		{
			return this->capacity_ + this->rear_ - this->front_;
			// return this->capacity_ - (this->front_ - this->rear_);
		}
        ...
        // return this->rear_ % capacity_ - this->front_ % capacity_;
	}
	...
	void Resize() // 2배씩 증가
	{
		...
		// // for (size_t idx = 0; idx < this->capacity_ - 1; idx++)
		// // {
		// // 	temp[idx] = this->queue_[((front_) + idx)];
		// // }
		// std::memcpy(temp, this->queue_, sizeof(T) * this->capacity_);
		// delete [] this->queue_;
		// this->capacity_ = this->capacity_ * 2;
		// this->queue_ = temp;

		// # Hong lab way
		T* new_queue = new T[2 * this->capacity_];
	...
	void Enqueue(const T& item) // 맨 뒤에 추가, Push()
	{
		...
		// this->rear_ = (this->rear_  + 1);
        this->queue_[this->rear_] = item;
	}

    /*
        용량이 부족하다면 resize해서 할당된 공간을 줄인다
    */
